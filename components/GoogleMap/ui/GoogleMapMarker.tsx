...
import LocationMarker, { MarkerProps } from "./Marker";
...
const renderReactNode = (children: ReactNode) => {
...
const GoogleMapMarker = ({ isActive = true, position, label, body, href, id, icon }: GoogleMapMarkerProps) => {
     ...
     * renderReactNode 함수를 통해 렌더하므로 (e.g. renderReactNode(<Marker/>)) renderReactNode의 argument 컴포넌트 내부에서는 theme이 적용되지 않음. 
     * */
    ...
    //                 position: position,
    //                 title: label,
    //                 content: renderReactNode(<LocationMarker key={id} icon={icon} color={palette.primary.main} />),
    //                 zIndex: 0,
    //             });
    ...
    //             marker.content = renderReactNode(<LocationMarker icon={icon} color={palette.primary.light} />)
    //             marker.zIndex = 0
    //             // InfoWindow 표시 
    //             // infoWindow?.open({
    //             //     anchor: marker,
    ...
    //         }
    //         else {
    //             marker.content = renderReactNode(<LocationMarker icon={icon} color={palette.primary.main} />)
    //         }
    //     }
    // }, [isOutFoucsed, marker])
    ...
    return (
        isActive &&
        <AdvancedMarker position={position} zIndex={isActive ? 1 : 0} onClick={() => {
            setSelectedPlaceId(id);
        }}>
            <LocationMarker icon={icon} color={isOutFoucsed ? palette.primary.light : palette.primary.main} />
        </AdvancedMarker>
    );
};
