        ...
        if (marker === undefined) {
            const asyncSetMarker = async () => {
                ...
                const markerElement = new AdvancedMarkerElement({
                    position: position,
                    title: label,
                    content: renderReactNode(<Marker key={id} icon={icon} color={palette.primary.main} />),
                    zIndex: 0,
                });
    ...
    useEffect(() => {
        if (marker) {
            if (isOutFoucsed) {
                ...
                marker.content = renderReactNode(<Marker icon={icon} color={palette.primary.light} />)
                marker.zIndex = 0
                // InfoWindow 표시 
                // infoWindow?.open({
                //     anchor: marker,
                ...
                marker.content = renderReactNode(<Marker icon={icon} color={palette.primary.main} />)
            }
        }
    }, [isOutFoucsed, marker])
